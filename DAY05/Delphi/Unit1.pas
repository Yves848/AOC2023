...
//        sLine := format(' %s : %u', [pMaps[iMaps].name, seed2]);
...
begin
  ...
  begin
    result := format('Desination : [%u - %u] Source : [%u - %u]', [Coords[i].destination, (Coords[i].destination + Coords[i].L) -
        1, Coords[i].source, (Coords[i].source + Coords[i].L) - 1]);
  end;
end;
